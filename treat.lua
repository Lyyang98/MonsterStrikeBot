...
function PlayStrategy()
	...
	while (flag==true) do
		flag=multiColor(menu,90)
		if multiColor(menu,90)==true then			
			testokbutton()
			mSleep(600)
...
--BacktomainScreen
function Back2Index_main()
	local timer=0
	while(multiColor(indexorange)==false)and(timer<=90000) do
		if multiColor(friendinfo,90)==true then
			testokbutton()
			tap(474,396)
			mSleep(2000)
			tap(218,861)
...
function Back2Index_sub()
	...
	while(multiColor(indexorange)==false)and(timer<=90000) do
		...
		mSleep(1000)
		if multiColor(friendinfo,90)==true then
			x1,y1 = findMultiColorInRegionFuzzy( 0xffffff, "13|-2|0xffffff,4|11|0x000000,-9|8|0xffffff,-11|-11|0xffffff,-4|-6|0x141212,10|-10|0xffffff,13|-6|0x000000,-1|0|0xffffff", 90, 148, 261, 481, 798)
			tap(468,y1+62)
			mSleep(2000)
			tap(218,861)
			mSleep(2000)
...
UIEdit("bg","起始序号","",15,"left","255,0,0")--返回值为字符串，若文本框内容为：测试，则返回 edt == "测试"
UIShow()
role=1
...
elseif role==2 then
	...
	while true do
		...
		Back2Index_sub()
		gp=gp+1
		Account_switch(bg+gp*3)
		Launch_index()
	end
elseif role==3 then
	Account_switch(1+bg+gp*3)
	Launch_index()
	while true do
		Index2join()
		PlayStrategy()
